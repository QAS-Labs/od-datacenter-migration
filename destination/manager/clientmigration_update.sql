...
\set clientid 18555,25369,25749
...
create index if not exists idx_tmp_test_case_results_customfieldvalues on test_case_results(id) where clientid IN (:clientid) and customfieldvalues is not null;
drop table if exists ztmpclientmigration.test_case_results_customfieldvalues_mig185552536925749;
create table ztmpclientmigration.test_case_results_customfieldvalues_mig185552536925749 as
...
from ztmpclientmigration.test_case_results_customfieldvalues_mig185552536925749 t
where
...
drop table if exists ztmpclientmigration.test_case_results_aud_customfieldvalues_mig185552536925749;
create table ztmpclientmigration.test_case_results_aud_customfieldvalues_mig185552536925749 as
...
set customfieldvalues = t.newvalue
from ztmpclientmigration.test_case_results_aud_customfieldvalues_mig185552536925749 t
where
...
--automation_cancelled_executions, projectid
update automation_cancelled_executions a
set projectid = z.id
from projects z
where a.projectid = z.fromid
...
and a.fromclientid IN (:clientid)
and a.fromclientid = z.fromclientid;
create index if not exists idx_tmp_test_step_parameter_values_testrunid on test_step_parameter_values(testrunid) where clientid IN (:clientid);
update test_step_parameter_values a
set testrunid = z.id
from test_case_run z
where a.testrunid = z.fromid
...
and a.clientid IN (:clientid)
and a.clientid = z.clientid;
drop index if exists idx_tmp_test_step_parameter_values_testrunid;
create index if not exists idx_tmp_test_step_parameter_values_teststepparameterid on test_step_parameter_values(teststepparameterid) where clientid IN (:clientid);
create index if not exists idx_tmp_test_step_parameters_fromid_id on test_step_parameters (fromid,id) where fromclientid IN (:clientid);
update test_step_parameter_values a
...
and a.fromclientid = z.fromclientid
and z.fromclientid IN (:clientid);
drop index if exists idx_tmp_test_step_parameter_values_teststepparameterid;
drop index if exists idx_tmp_test_step_parameters_fromid_id;
update test_step_result_defects a
...
\copy (SELECT clientid as fromclientid, sessionid, fromid FROM session WHERE clientid IN (18555,25369,25749) and fromid is not null and sessionid != fromid) to 'newid_session.dat'
...
where a.objectid = z.fromid
and a.clientid IN (18555,25369,25749)
and a.fromclientid IN (18555,25369,25749)
and a.fromclientid = z.fromclientid
